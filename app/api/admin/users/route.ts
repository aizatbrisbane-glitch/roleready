...
export async function GET(request: Request) {
  ...
  const [rpcResult, { data: profiles }, { data: entitlements }] = await Promise.all([
    ...
    admin.from("profiles").select("id, name, created_at, monthly_generations_used, monthly_generations_reset_at, newsletter_subscribed, attr_source, attr_medium, attr_campaign, attr_landing_page, attr_landed_at"),
    admin
      .from("entitlements")
  ...
  const profileById: Record<string, { name: string | null; created_at: string; monthlyUsed: number; monthlyResetAt: string | null; newsletterSubscribed: boolean; attrSource: string | null; attrMedium: string | null; attrCampaign: string | null; attrLandingPage: string | null; attrLandedAt: string | null }> = Object.fromEntries(
    (profiles ?? []).map((p) => [p.id as string, {
      ...
      newsletterSubscribed: (p.newsletter_subscribed as boolean) ?? false,
  ...
  let users = authEmails.map((u) => {
    ...
    return {
      ...
      newsletterSubscribed: prof?.newsletterSubscribed ?? false,
