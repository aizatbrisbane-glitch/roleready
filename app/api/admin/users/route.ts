...
export async function GET(request: Request) {
  ...
  const [rpcResult, { data: profiles }, { data: entitlements }] = await Promise.all([
    ...
    admin.from("profiles").select("id, name, created_at, monthly_generations_used, monthly_generations_reset_at, attr_source, attr_medium, attr_campaign"),
    admin
      .from("entitlements")
  ...
  const profileById: Record<string, { name: string | null; created_at: string; monthlyUsed: number; monthlyResetAt: string | null; attrSource: string | null; attrMedium: string | null; attrCampaign: string | null }> = Object.fromEntries(
